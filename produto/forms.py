...
class AbrirComandaForm(forms.Form):
    ...
    def clean(self):
        ...
        if not telefone_cliente and not nome_cliente:
            raise forms.ValidationError("Você deve fornecer um número de telefone ou um nome de cliente.")
        
        # Verifica se a comanda já está aberta para o telefone ou nome fornecido
        if telefone_cliente:
            if Tab.objects.filter(telefone_cliente=telefone_cliente, aberta=True).exists():
                raise forms.ValidationError("Já existe uma comanda aberta para este número de telefone.")

        if nome_cliente:
            if Tab.objects.filter(nome_cliente=nome_cliente, aberta=True).exists():
                raise forms.ValidationError("Já existe uma comanda aberta para este nome de cliente.")
