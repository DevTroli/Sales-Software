...
class ItemCompraForm(forms.Form):
    ...
    #     'placeholder': ' ',
    # }))
    quantidade = forms.IntegerField(min_value=1, widget=forms.NumberInput(attrs={
        'class': 'mb-2 block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none text-white border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer',
        'placeholder': ' Quantas Unidades ? ',
    }))
    codigo_barra = forms.CharField(label="Código de Barras", max_length=16, required=False, widget=forms.TextInput(attrs={
        'class': 'mb-2 block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-300 appearance-none text-white border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer',
        'placeholder': 'Digite o Codigo de Barras ',
    }))
    nome_produto = forms.CharField(label="Nome do Produto", max_length=100, required=False, widget=forms.TextInput(attrs={
    ...
    def clean(self):
        ...
        if not codigo_barra and not nome_produto:
            raise forms.ValidationError("Você deve fornecer um ID, um código de barras ou um nome do produto.")
